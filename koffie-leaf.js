(function() {
    ...
    function validationInput(e) {
        ...
        if (target === formContact.activeElement ) {
    ...
    if (contactForm) {
        ...
        if (contactForm.addEventListener) {
            ...
            contactForm.attachEvent('onkeyup', function(e) {
                validationInput(e);
            });
        }
    }
    
    /*Validates for characters, empty inputs or incorrect input.
	  Also keeps text that is written without refreshing*/
    ...
    function validation(e) {
		...
		if (textInput.trim().length === 0 || textArea.trim().length === 0 ) {
            alert('Fill in contact page');
			return;
        }
        ...
        if (textInput !== '' && textArea !== '' && emailAddress.value === (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(emailAddress.value))) {
